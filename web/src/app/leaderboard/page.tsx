...
const LeaderboardRow = memo(function LeaderboardRow({ row }: { row: RankedRow }) {
  ...
  return (
    <tr onClick={() => { window.location.href = `/leaderboard/providers/${encodeURIComponent(provider)}${row.run_id ? `?run=${encodeURIComponent(row.run_id)}` : ""}`; }}
      ...
      <td className="px-3 py-3 font-mono text-[rgba(226,232,240,0.6)] text-[11px]">{row._rank}</td>
      <td className="px-3 py-3">
        <div className="flex items-center gap-2.5">
          <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-[12px] font-bold"
            style={{ backgroundColor: aColor + "22", color: aColor }}>{aChar}</span>
          <div className="flex flex-col gap-0.5 leading-tight">
            <span className="text-[13px] font-medium text-[rgba(226,232,240,0.92)] leading-none">{provider}</span>
            <span className="text-[10px] text-[var(--nexus-muted)]">{row.source ?? "—"}</span>
          </div>
        </div>
      </td>
      <td className="px-3 py-3 font-mono text-[rgba(226,232,240,0.75)] text-[11px]">
      ...
      <td className="px-3 py-3">
        {row.ticker
          ? <span className="rounded border border-[rgba(138,149,166,0.15)] bg-[rgba(6,8,11,0.35)] px-2 py-0.5 text-[10px] font-mono text-[rgba(226,232,240,0.7)]">{row.ticker}</span>
          : <span className="text-[var(--nexus-muted)]">—</span>}
      </td>
      <td className={`px-3 py-3 text-right font-semibold tabular-nums text-[12px] ${pos ? "text-[rgba(0,212,170,0.92)]" : "text-[rgba(242,92,84,0.95)]"}`}>{fmtPct(ret, 2)}</td>
      <td className="px-3 py-3 text-right font-mono tabular-nums text-[11px] text-[rgba(226,232,240,0.75)]">{fmtNum(row.sharpe, 2)}</td>
      <td className="px-3 py-3 text-right font-mono tabular-nums text-[11px] text-[rgba(226,232,240,0.75)]">{fmtNum(row.win_rate, 1)}</td>
      <td className="px-3 py-3 text-right font-mono tabular-nums text-[11px] text-[rgba(226,232,240,0.75)]">{fmtInt(row.trade_count)}</td>
      <td className="px-3 py-3 text-right font-mono tabular-nums text-[11px] text-[rgba(242,92,84,0.8)]">{fmtNum(row.max_drawdown_pct, 1)}</td>
      <td className="px-3 py-3 text-right font-mono tabular-nums text-[11px] text-[rgba(226,232,240,0.75)]">{fmtNum(row.profit_factor, 2)}</td>
      <td className="px-3 py-3 text-right text-[11px] text-[var(--nexus-muted)]">{fmtTsRel(row.ts)}</td>
    </tr>
  );
});
            ...
            {loading && filtered.length === 0 && !error ? (
              ...
              <section className="overflow-hidden rounded-xl border border-[color:var(--nexus-card-stroke)] bg-[var(--nexus-panel)]/45">
                <div className="overflow-auto max-h-[calc(100vh-200px)]">
                  <table className="w-full min-w-[780px] border-separate border-spacing-0 text-left text-[12px]">
                    <thead className="sticky top-0 z-10 bg-[rgba(6,8,11,0.9)] backdrop-blur">
                      <tr className="text-[10px] uppercase tracking-[0.16em] text-[var(--nexus-muted)]">
                        <th className="px-2.5 py-2.5 w-9">#</th>
                        <th className="px-2.5 py-2.5">Provider</th>
                        <th className="px-2.5 py-2.5">Run</th>
                        <th className="px-2.5 py-2.5">Ticker</th>
                        <th className="px-2.5 py-2.5 text-right">Return</th>
                        <th className="px-2.5 py-2.5 text-right">Sharpe</th>
                        <th className="px-2.5 py-2.5 text-right">W Rate</th>
                        <th className="px-2.5 py-2.5 text-right">Trades</th>
                        <th className="px-2.5 py-2.5 text-right">MDD</th>
                        <th className="px-2.5 py-2.5 text-right">Profit</th>
                        <th className="px-2.5 py-2.5 text-right">When</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filtered.length === 0 && !loading ? (
                        <tr>
                          <td className="px-3 py-8 text-center text-[12px] text-[var(--nexus-muted)]" colSpan={11}>
                            No results yet.
                          </td>
                        </tr>
                      ) : null}
