...
export function BacktestBarTimeline({
  ...
  const groups = useMemo(
    () => groupBacktestMessageLog(entries, streaming ?? false),
    [entries, streaming],
  );
    ...
    >
      {streaming && entries.length === 0 ? (
        <p
          className={
            compact
              ? "text-[10px] text-[var(--nexus-muted)]"
              : "text-[13px] leading-relaxed text-[var(--nexus-muted)]"
          }
        >
          Waiting for events…
        </p>
        ...
        {groups.map((g) => {
          ...
          return (
            ...
            >
              ...
              >
                <span
                  className={`shrink-0 text-[var(--nexus-muted)] ${compact ? "mt-px" : "mt-1"}`}
                >
                  {isOpen ? (
                    <ChevronDown size={compact ? 14 : 18} strokeWidth={2} />
                  ) : (
                    <ChevronRight size={compact ? 14 : 18} strokeWidth={2} />
                  )}
                </span>
                  ...
                  {compact ? (
                    <>
                      <div className="flex flex-wrap items-center gap-x-1.5 gap-y-0.5">
                        <span className="font-mono text-[11px] font-semibold tracking-tight text-[var(--nexus-glow)]">
                          {g.title}
                        </span>
                        {g.barTimeUtc ? (
                          <>
                      ...
                      {!isOpen && summaryLine ? (
                        <p
                          className="mt-0.5 truncate font-mono text-[9px] leading-snug text-[var(--nexus-muted)]"
                          title={summaryLine}
                        >
                          {summaryLine}
                        </p>
                    ...
                    <>
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                        <span className="font-mono text-[13px] font-semibold tracking-tight text-[var(--nexus-glow)]">
                          {g.title}
                        </span>
                        {g.barTimeUtc ? (
                          <span className="font-mono text-[12px] text-[var(--nexus-muted)]">
                            {g.barTimeUtc}
                          </span>
                        ) : null}
                        {statusPill(g.status)}
                      ...
                      {!isOpen ? (
                        <div className="mt-2.5 flex max-h-[3.25rem] flex-wrap gap-2 overflow-hidden">
                          {lines.slice(0, 5).map(({ actor, line }) => (
                            <span
                              key={actor}
                              className={`inline-block max-w-full font-mono text-[11px] leading-snug text-[var(--nexus-text)] ${moduleChipClass()}`}
                            >
                              <span className="font-semibold text-[var(--nexus-glow)]">
                                {actor}
                              </span>
                          ...
                          {lines.length > 5 ? (
                            <span className="self-center font-mono text-[10px] text-[var(--nexus-muted)]">
                              +{lines.length - 5}
                            </span>
                          ) : null}
                        </div>
              ...
              {isOpen ? (
                <div
                  className={`border-t border-[var(--nexus-rule-soft)] bg-[var(--nexus-bg)]/35 ${compact ? "px-2 py-1.5" : "px-3 py-4 sm:px-5 sm:py-5"}`}
                >
                  <div className={detailGrid}>
                      ...
                      <div className="flex min-h-0 min-w-0 flex-col">
                        ...
                        <div
                          ...
                          className={`nexus-scroll min-h-0 overflow-y-auto overflow-x-hidden overscroll-y-contain ${
                            compact
                              ? "max-h-[min(50vh,28rem)] space-y-2 pr-0.5"
                              : "max-h-[min(55vh,32rem)] space-y-3 pr-1"
                          }`}
                        >
                          {barTraces.map((tr, i) => (
                            <AgentTraceCard
                    ...
                    <div
                      className={`flex min-h-0 min-w-0 flex-col ${logColSpan} ${
                        hasCoT
                          ? "border-t border-[var(--nexus-rule-soft)] pt-3 min-[720px]:border-t-0 min-[720px]:pt-0"
                          : ""
                      }`}
                    >
                      <h4
                      ...
                      <div
                        ...
                        className={`nexus-scroll min-h-0 overflow-y-auto overflow-x-auto overscroll-y-contain rounded-md border border-[color:var(--nexus-card-stroke)] bg-[var(--nexus-bg)]/50 ${
                          compact
                            ...
                            : "max-h-[min(55vh,32rem)] rounded-xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]"
                        }`}
                      >
