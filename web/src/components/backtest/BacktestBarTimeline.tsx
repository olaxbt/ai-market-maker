  ...
  return (
    ...
    >
      {streaming && entries.length === 0 ? (
        <p className={compact ? "text-[10px] text-[var(--nexus-muted)]" : "text-[13px] leading-relaxed text-[var(--nexus-muted)]"}>Waiting for events…</p>
      ) : null}
      {!streaming && entries.length === 0 ? (
        <p className={compact ? "text-[10px] leading-snug text-[var(--nexus-muted)]" : "max-w-prose text-[13px] leading-relaxed text-[var(--nexus-muted)]"}>
      ...
      <div className={compact ? "space-y-0.5" : "space-y-2"}>
        {groups.map((g) => {
          ...
          return (
            <div
              key={g.key}
              className={`border border-[color:var(--nexus-card-stroke)] bg-[var(--nexus-panel)]/75 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.03)] ${compact ? "rounded-md" : "rounded-xl"}`}
            >
              <button
                type="button"
                onClick={() => toggle(g.key)}
                className={`sticky top-0 z-10 flex w-full items-start gap-1.5 border-b border-[var(--nexus-rule-soft)] bg-[var(--nexus-panel)]/95 text-left shadow-[0_6px_12px_-8px_rgba(0,0,0,0.45)] backdrop-blur-sm transition hover:bg-[var(--nexus-surface)]/55 ${compact ? "px-2 py-1.5" : "gap-3 px-3 py-3 sm:px-4 sm:py-3.5"}`}
              >
                  ...
                  <div className={detailGrid}>
                    {hasCoT ? (
                      <div className="flex min-h-0 min-w-0 flex-col">
                        <h4
                          className={`font-mono font-semibold uppercase tracking-[0.15em] text-[var(--nexus-text)] ${compact ? "mb-1.5 text-[9px]" : "mb-3 text-[11px] tracking-[0.18em]"}`}
                        >
                          Chain of thought
                        </h4>
                        <div
                          ...
                          className={`nexus-scroll min-h-0 overflow-y-auto overflow-x-hidden overscroll-y-contain ${compact ? "max-h-[min(50vh,28rem)] space-y-2 pr-0.5" : "max-h-[min(55vh,32rem)] space-y-3 pr-1"}`}
                        >
                          {barTraces.map((tr, i) => (
                    ...
                    <div
                      className={`flex min-h-0 min-w-0 flex-col ${logColSpan} ${hasCoT ? "border-t border-[var(--nexus-rule-soft)] pt-3 min-[720px]:border-t-0 min-[720px]:pt-0" : ""}`}
                    >
                      <h4
                        className={`font-mono font-semibold uppercase tracking-[0.15em] text-[var(--nexus-text)] ${compact ? "mb-1.5 text-[9px]" : "mb-3 text-[11px] tracking-[0.18em]"}`}
                      >
                      ...
                      {!compact ? (
                        <p className="mb-3 text-[12px] leading-relaxed text-[var(--nexus-muted)]">Events oldest → newest. Wide payloads scroll inside each row.</p>
                      ) : null}
                      <div
                        role="region"
                        aria-label="Event log for this bar"
                        className={`nexus-scroll min-h-0 overflow-y-auto overflow-x-auto overscroll-y-contain rounded-md border border-[color:var(--nexus-card-stroke)] bg-[var(--nexus-bg)]/50 ${compact ? "max-h-[min(50vh,28rem)]" : "max-h-[min(55vh,32rem)] rounded-xl shadow-[inset_0_1px_0_0_rgba(255,255,255,0.04)]"}`}
                      >
                        <div className="min-w-0 divide-y divide-[var(--nexus-rule-soft)]">
                          {g.entries.map((row, idx) => (
                            <div
                              key={row.seq}
                              className={`select-text ${compact ? "px-2 py-2" : "px-3 py-3.5 sm:px-4 sm:py-4"} ${idx % 2 === 0 ? "bg-[var(--nexus-surface)]/15" : "bg-[var(--nexus-bg)]/25"}`}
                            >
                              <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">
                                <span
                                  className={`inline-flex items-center rounded border border-[color:var(--nexus-card-stroke)] bg-[var(--nexus-panel)]/60 font-mono tabular-nums text-[var(--nexus-text)] ${compact ? "px-1 py-px text-[9px]" : "px-2 py-0.5 text-[11px]"}`}
                                >
                                  {formatTraceTime(row.ts)}
                                </span>
                                <span
                                  className={`inline-flex max-w-full items-center rounded border border-[var(--nexus-glow)]/35 bg-[var(--nexus-glow)]/10 font-mono font-medium text-[var(--nexus-glow)] ${compact ? "px-1 py-px text-[9px]" : "px-2 py-0.5 text-[11px]"}`}
                                >
                                  {row.actor_id}
                                </span>
                                <span
                                  className={`inline-flex items-center rounded border border-[color:var(--nexus-card-stroke)] font-mono uppercase tracking-wide text-[var(--nexus-muted)] ${compact ? "px-1 py-px text-[8px]" : "px-2 py-0.5 text-[10px]"}`}
                                >
                                  {row.kind}
                                </span>
                              </div>
                              <div
                                className={`mt-1.5 overflow-x-auto rounded border border-[color:var(--nexus-card-stroke)] bg-black/25 ${compact ? "px-2 py-1.5" : "mt-3 px-3 py-3 sm:px-4"}`}
                              >
                                <p
                                  className={`min-w-0 whitespace-pre-wrap break-words font-mono text-[var(--nexus-text)] ${compact ? "text-[11px] leading-snug" : "text-[13px] leading-[1.65]"}`}
                                >
                                  {displayMessage(row)}
                                </p>
                              </div>
