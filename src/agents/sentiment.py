...
class SentimentAgent:
    def __init__(self):
        ...
        self.prompt = PromptTemplate(
            ...
            template=(
                "Analyze sentiment for {ticker} based on tweets: {tweets}. "
                "Provide a sentiment score (0-100, 100=very bullish)."
            ),
        )
    ...
    def analyze(self, ticker: str) -> Dict:
        try:
            ...
            if self.llm:
                analysis = self.llm.generate(self.prompt.format(ticker=ticker, tweets=tweets))
                try:
