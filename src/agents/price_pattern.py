        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a financial analyst providing trading insights.",
                    },
...
class PricePatternAgent:
    ...
    def analyze(self, ticker: str, market_data: Dict) -> Dict:
        try:
            ...
            closes = [candle[4] for candle in ohlcv if isinstance(candle[4], (int, float))]
            ...
            formatted_indicators = {
                ...
                if not np.isnan(indicators["sma"])
                else "unavailable",
                "bb_upper": f"{indicators['bb_upper']:.2f}"
                if not np.isnan(indicators["bb_upper"])
                else "unavailable",
                "bb_lower": f"{indicators['bb_lower']:.2f}"
                if not np.isnan(indicators["bb_lower"])
                else "unavailable",
                "bb_mid": f"{indicators['bb_mid']:.2f}"
                if not np.isnan(indicators["bb_mid"])
                else "unavailable",
            }
            ...
            if self.llm:
                ...
                analysis = (
                    f"Technical analysis for {ticker}: RSI={formatted_indicators['rsi']}, "
                    f"SMA={formatted_indicators['sma']}, "
                    f"Bollinger Bands (Upper={formatted_indicators['bb_upper']}, "
                    f"Lower={formatted_indicators['bb_lower']}, "
                    f"Mid={formatted_indicators['bb_mid']})"
                )
            ...
            logger.error(f"Price pattern analysis error for {ticker}: {str(e)}")
            return {
                "ticker": ticker,
                "indicators": {
                    "rsi": "unavailable",
