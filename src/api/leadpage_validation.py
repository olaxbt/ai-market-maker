...
def ticker_exists(ticker: str) -> bool:
    ...
    Returns True for known pairs, True for unknown (permissive), False for
    known-delisted.
    """
...
def validate_result(payload: dict) -> list[str]:
    ...
    submitted_at = payload.get("submitted_at", 0)
    if isinstance(submitted_at, int) and submitted_at > 0:
        age = now_ts - submitted_at
        if age < 0:
    ...
    if isinstance(initial, (int, float)) and isinstance(final, (int, float)) and initial > 0:
        ...
        if isinstance(total_return_pct, (int, float)):
            ...
            if diff > 20 and abs(total_return_pct) > 1:
                errors.append(
