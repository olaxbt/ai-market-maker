...
def calculate_technical_indicators(prices: List[float], period: int = 14) -> Dict:
    ...
    try:
        if not prices or len(prices) < period + 1:
            logger.warning(f"Insufficient data: {len(prices)} prices, need {period + 1}")
            return {
                "rsi": 50.0,
                "sma": np.nan,
                "bb_upper": np.nan,
        ...
        # Validate prices
        prices = np.array(
            [float(p) for p in prices if isinstance(p, (int, float)) and not np.isnan(p)]
        )
        if len(prices) < period + 1:
            logger.warning(f"Valid prices reduced to {len(prices)}, need {period + 1}")
            return {
                "rsi": 50.0,
                "sma": np.nan,
                "bb_upper": np.nan,
        ...
        logger.debug(
            f"Indicators: RSI={result['rsi']:.2f}, SMA={result['sma']:.2f}, "
