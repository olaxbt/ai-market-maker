...
export default function StudioPage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = safeJsonParse<StudioSession[]>(
        localStorage.getItem(STUDIO_SESSIONS_KEY),
      );
      const savedActive = localStorage.getItem(STUDIO_ACTIVE_SESSION_KEY);
      if (Array.isArray(saved) && saved.length > 0) {
        setSessions(saved);
        setActiveId(
          savedActive && saved.some((s) => s.id === savedActive)
            ? savedActive
            : saved[0].id,
        );
        return;
  ...
  async function send(overrideText?: string) {
    ...
    try {
      ...
      const steps: Step[] = Array.isArray((data as any)?.steps)
        ? (data as any).steps
        : [];
          ...
          setSessions((prev) =>
            prev.map((ss) =>
              ss.id === sessionId
                ? {
                    ...ss,
                    messages: [...ss.messages, { role: "assistant", text: s.text }],
                  }
                : ss,
            ),
          );
        ...
        <aside className="min-h-0 border-r border-[rgba(15,23,42,0.08)] bg-[rgba(255,255,255,0.55)] p-4 backdrop-blur">
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <div className="mt-1 text-[12px] font-semibold text-[rgba(15,23,42,0.82)]">
                Chats
              </div>
            </div>
            <button
          ...
          <div className="flex h-full min-h-0 flex-col">
            <div className="shrink-0 border-b border-[rgba(15,23,42,0.08)] px-10 py-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="text-[10px] text-[rgba(15,23,42,0.55)]">
                  {busy ? "Thinking…" : "Ready"}
                </div>
              </div>
            </div>
            ...
            <div className="shrink-0 px-10 pb-10">
              <div className="mx-auto w-full max-w-[980px] rounded-[26px] border border-[rgba(15,23,42,0.10)] bg-[rgba(255,255,255,0.86)] px-4 py-4 shadow-[0_18px_50px_rgba(2,6,23,0.12)] backdrop-blur">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    placeholder={
                      busy ? "Thinking…" : "Ask: onboarding, publish to leaderboard, or a strategy idea…"
                    }
                    disabled={busy}
              ...
              <div className="mx-auto mt-3 w-full max-w-[980px] text-[11px] text-[rgba(15,23,42,0.55)]">
                Tip: use <b>Get Started</b> to run locally; use <b>Control</b> to run backtests and
                inspect receipts.
              </div>
            </div>
          </div>
...
function RailTab({
  ...
  return (
    ...
    >
      <span className={active ? "text-[rgba(0,160,130,0.95)]" : "text-[rgba(15,23,42,0.55)]"}>
        {icon}
      </span>
      <span>{label}</span>
    </Link>
